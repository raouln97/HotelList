...
import { Card, CardContent, Typography, CardMedia, Rating, Collapse, Divider, Box , Badge, Accordion, AccordionSummary, AccordionDetails, Grid, TextField} from '@mui/material';
...
const HotelDetails = ({ hotel }) => {
  ...
  const handleReviewsSearch = (event) => {
    ...
    if (currentValue !== '') {
      setFilteredReviews(hotel.reviews.filter((review) => {
        if (review.description.toLowerCase().includes(currentValue.toLowerCase())){
          return true
        }else if(review.title.toLowerCase().includes(currentValue.toLowerCase())){
          return true
        }
        return false
      }
  ...
  return (
    <Card sx={{ display: 'flex', marginBottom: 2, flexDirection: 'column', backgroundColor: 'rgba(255, 255, 255, 0.97)' }}>
      ...
      <CardContent sx={{ flex: '1 0 auto' }}>
        ...
        <Grid item container flexDirection='row' display='flex' alignItems='center'>
          <Grid item xs={9}>
      ...
      <Box sx={{ padding: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Badge sx={{ cursor: 'pointer' }} onClick={handleReviewsToggle} badgeContent={hotel.rating} color={reviewsOpen && hotel.reviews.length > 0  ? "primary" : "secondary"}>
          <HotelIcon /> 
        </Badge>
